...
def bookRegister():
    book_name= bookInfo1.get()
    author = bookInfo2.get()
    publisher = bookInfo3.get()

    book = {'book_name':book_name,'author':author,'publisher':publisher}
    headers = {
        'Content-type': 'application/json',
        'Accept': 'application/json'
    }

    book_data=json.dumps(book)

    response=requests.post('http://127.0.0.1:8000/books',data=book_data,headers=headers)
    print(response)
...
def addBook():
    ...
    lb1 = Label(labelFrame, text="Book Name : ", bg='black', fg='white', font=('Courier', 12))
    lb1.place(relx=0.05, rely=0.2, relheight=0.08)

    bookInfo1 = Entry(labelFrame)
    bookInfo1.place(relx=0.3, rely=0.2, relwidth=0.62, relheight=0.08)

    lb2 = Label(labelFrame, text="Author : ", bg='black', fg='white', font=('Courier', 12))
    lb2.place(relx=0.05, rely=0.35, relheight=0.08)

    bookInfo2 = Entry(labelFrame)
    bookInfo2.place(relx=0.3, rely=0.35, relwidth=0.62, relheight=0.08)

    lb3 = Label(labelFrame, text="Publisher : ", bg='black', fg='white', font=('Courier', 12))
